...
SM_AliasFunctions[0] = function(body)
    return body;
end
...
function FindSpell(spell, rank)
    ...
    for k, book in booktype do
        while spell do
            ...
            if (string.lower(s) == string.lower(spell)) then
                ys = true;
            end
            if ((r == rank) or (r and rank and string.lower(r) == string.lower(rank))) then
                yr = true;
            end
...
function GetSuperMacroInfo(superName, code)
    if (not superName or not SM_SUPER[superName]) then
        return ;
    end
